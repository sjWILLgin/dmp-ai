...
(async () => {
  ...
  if (!fs.existsSync(chromePath)) {
    ...
    console.error('Set environment variable CHROME_PATH to the path of your Chrome/Chromium binary.');
    process.exit(2);
  }
  ...
  page.on('console', (msg) => {
    try {
      const args = msg.args().map(async (a) => {
        try {
      ...
      Promise.all(args).then((vals) => {
        consoleLogs.push({ type: msg.type(), text: vals.join(' '), location: msg.location() });
      });
    } catch (e) {
  ...
  // no explicit sleep; rely on networkidle2 above
